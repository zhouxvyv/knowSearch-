$(function () {
    ...
    $(document).ready(function () {
            ...
            swiperHtml+='<h2 class=" pb-2 pt-3 mb-2 border-bottom">\n' +
                '                        在线考试\n' +
                '                        <p class="text-muted fs-5" style="margin-top: 10px">' + testName + ' 高等数学</p>\n' +
                '                    </h2>';
            $('#dpTitle').html(swiperHtml);
        }
    )
    $(document).ready(
        $.post("/getExamTopicList", {userId: userID, testId: getUrlParam("testId")}, function (data) {
            if (data.result) {
                ...
                queList.map(function (item, index) {
                    swiperHtml+='<div class="clearfix">\n' +
                        '                        <i class="que-li" id="que_' + index+1 + '">' + index+1 + '</i>\n' +
                        '                        <i class="que-type">'+item.topicType+'</i>\n' +
