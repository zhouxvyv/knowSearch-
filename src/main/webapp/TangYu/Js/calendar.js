(function (root, factory) {
    ...
    var defaults = {
        ...
        TEMPLATE = [
            ...
            '<span class="prev" title="上一月" onclick="reloadDays()"  data-calendar-arrow-date>{prev}</span>',
            '<span class="next" title="下一月" onclick="reloadDays2()" data-calendar-arrow-date>{next}</span>',
            '</div>',
            '</div>',
            ...
            '<div class="calendar-arrow">',
            '<span class="prev" title="上一年" onclick="reloadDays()" data-calendar-arrow-month>{prev}</span>',
            '<span class="next" title="下一年" onclick="reloadDays()" data-calendar-arrow-month>{next}</span>',
            '</div>',
            '</div>',
    ...
    Date.prototype.myIsSame = function (y, m) {
        if (isDate(y)) {
            var dt = y;
            y = dt.getFullYear();
            m = dt.getMonth() + 1;
        }
        return this.getFullYear() === y && this.getMonth() + 1 === m;
    }
    ...
    Calendar.prototype = {
        ...
        getDaysHtml: function (y, m) {
            ...
            for (var p = prevDaysNum - prevDiff + 1; p <= prevDaysNum; p++, count++) {
                $days.append(this.getDayItem(prevM.y, prevM.m, p, PREV_FLAG, x));
            }

            for (var c = 1; c <= daysNum; c++, count++) {
                $days.append(this.getDayItem(year, month, c, CURR_FLAG, x));
            }

            for (var n = 1, nl = 42 - count; n <= nl; n++) {

                $days.append(this.getDayItem(nextM.y, nextM.m, n, NEXT_FLAG, x));
            }
        ...
        event: function () {
            ...
            _this.$element.on('click', '[' + ITEM_DAY + ']', function () {
                var d = parseInt(this.innerHTML),
                    ...
                    topicConstructURL = "/QueryDplistSpringMVC",
                    userID = 2;
                ...
                $.post(topicConstructURL, {userId: userID, date: day}, function (data) {
                    if (data.result) {
                        var URL = 'practice.html' + '?date=' + day;
